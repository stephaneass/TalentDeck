<div>
    <!-- start page title -->
    <x-breadcrumb title="Experiences"/>
    <!-- end page title -->
    @include('modals/experiences/add')
                            ...
                            <table class="table align-middle table-nowrap table-striped-columns mb-0">
                                ...
                                <tbody>
                                    @foreach ($experiences as $item)
                                        @include('livewire.experiences.item')
                                    @endforeach
                                </tbody>
                            </table>
                            <div class="d-flex justify-content-between mt-2">
                                <div> </div>
                                <div class="float-right">{{ $experiences->links() }}</div>
                            </div>
...
@push('scripts')
    ...
    <script>
        Livewire.on('hideAddExperienceModal', function(){
            $('#hideAddExperienceButton').trigger('click');
        })
